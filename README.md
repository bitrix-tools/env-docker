...
Таким образом Redis будет использовать контейнер с версией `7.4.6`.
...
- база данных MySQL:
  - используем стабильный образ `percona/percona-server:8.0.43` / `percona/percona-server:8.4.6`
  - добавляем слоем сверху конфигурацию бд
  - собираем `bitrix24/percona-server:8.0.43-v1-rhel` / `bitrix24/percona-server:8.4.6-v1-rhel`
- веб-сервер:
...
- интерпретатор PHP-кода:
  ...
  - собираем `bitrix24/php:8.2.29-fpm-v1-alpine` / `bitrix24/php:8.3.26-fpm-v1-alpine` / `bitrix24/php:8.4.13-fpm-v1-alpine`
- поиск:
...
- сервис для бесплатных SSL-сертификатов от `LetsEncrypt`:
  - используем стабильный образ `goacme/lego:v4.26.0`
  - добавляем логику слоем сверху
  - собираем `bitrix24/lego:4.26.0-v1-alpine`
...
```bash
cd env-docker/sources/bxphp8326/
docker buildx build --platform linux/arm64,linux/amd64 --provenance=false -f Dockerfile -t bitrix24/php:8.3.26-fpm-v1-alpine --no-cache .
```

- `bitrix24/php` для версии `8.4.x`:
```bash
cd env-docker/sources/bxphp8413/
docker buildx build --platform linux/arm64,linux/amd64 --provenance=false -f Dockerfile -t bitrix24/php:8.4.13-fpm-v1-alpine --no-cache .
```
...
```bash
cd env-docker/sources/bxpercona8043/
docker buildx build --platform linux/arm64,linux/amd64 --provenance=false -f Dockerfile -t bitrix24/percona-server:8.0.43-v1-rhel --no-cache .
```
...
- `bitrix24/lego`:
```bash
cd env-docker/sources/bxlego4260/
docker buildx build --platform linux/arm64,linux/amd64 --provenance=false -f Dockerfile -t bitrix24/lego:4.26.0-v1-alpine --no-cache .
```
