...
Таким образом Redis будет использовать контейнер с версией `8.4.7`.
...
- веб-сервер:
  ...
  - собираем `bitrix24/nginx:1.30.5-v1-alpine`
...
- `bitrix24/nginx`:
```bash
cd env-docker/sources/bxnginx1305/
docker buildx build --platform linux/arm64,linux/amd64 --provenance=false -f Dockerfile -t bitrix24/nginx:1.30.5-v1-alpine --no-cache .
```
...
Модули собираются на базе стабильного образа `nginx:1.30.5-alpine-slim`, используя официальный образ Nginx с [DockerHub](https://hub.docker.com/):
- `Nginx`: https://hub.docker.com/_/nginx
...
Скачиваем файл для версии 1.30.5 по ссылке: [https://raw.githubusercontent.com/nginx/docker-nginx/a16f1329e13e7273c4103f75d863ca625b75109e/stable/alpine/Dockerfile](https://raw.githubusercontent.com/nginx/docker-nginx/a16f1329e13e7273c4103f75d863ca625b75109e/stable/alpine/Dockerfile)

Модифицируем файл, добавляем нужные модули по списку выше и служебную часть. Пример всех изменений файла для версии 1.30.5 можно найти в папке `/sources/bxnginx1305modules/v1/`.

Запускаем сборку образа `nginx_modules`, указываем две архитектуры `amd64` и `arm64` в команде:

```bash
cd env-docker/sources/bxnginx1305modules/v1/
docker buildx build --platform linux/arm64,linux/amd64 --provenance=false -f Dockerfile -t bitrix24/nginx_modules:1.30.5-v1-alpine --no-cache .
```
...
```bash
docker run --platform=linux/amd64 -d --name=nginxmodules1305testingamd64 -it bitrix24/nginx_modules:1.30.5-v1-alpine
```

Для `arm64` выполняем команду:
```bash
docker run --platform=linux/arm64 -d --name=nginxmodules1305testingarm64 -it bitrix24/nginx_modules:1.30.5-v1-alpine
```
...
Для `amd64` выполняем команду:
```bash
docker container stop nginxmodules1305testingamd64 && docker container rm nginxmodules1305testingamd64
```

Для `arm64` выполняем команду:
```bash
docker container stop nginxmodules1305testingarm64 && docker container rm nginxmodules1305testingarm64
```
...
Механизм сборки для версии `1.30.5` можно найти в файле `/sources/bxnginx1305/Dockerfile`.
